    ...
    for epoch in range(args.n_epochs):
        ...
        for cnt, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            if args.use_relative_loss: # C.use_relative_loss = True
                ...
                sequences_predict_ = sequences_predict.reshape(b,n,18,3)
                dsequences_predict = gen_velocity(sequences_predict_)
                sequences_gt_ = sequences_gt.reshape(b,n,18,3)
                dsequences_gt = gen_velocity(sequences_gt_)
                dloss = mpjpe_error(dsequences_predict, dsequences_gt)*1000
                loss = loss + dloss
            else:
                loss = loss.mean()
        ...
        if (epoch+1)%1==0:
            print('----saving model-----')
            model_name_ = 'amass_3d_'+str(args.output_n)+'frames_ckpt_'+'epoch_'+str(epoch)
...
def test_mpjpe(model, args):
    ...
    if save_loss == 0:
      save_loss = accum_loss/n
    
    else:
    
      if accum_loss/n <= save_loss:
        save_loss = accum_loss/n
        
        print('----saving best model-----')
        model_name_ = 'amass_3d_'+str(args.output_n)+'frames_ckpt_'+'best'
        # torch.save(model.state_dict(),os.path.join(args.model_path,model_name_))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/content/drive/MyDrive/MotionMixer-main/MotionMixer-main/amass/AMASS_daraset', help='path to the unziped dataset directories(H36m/AMASS/3DPW)')  
    ...
    parser.add_argument('--root', default='/content/drive/MyDrive/MotionMixer-main/MotionMixer-main/amass/run_for_longging', type=str, help='root path for the logging') #'./runs'
    ...
    parser.add_argument('--model_path', type=str, default='/content/drive/MyDrive/MotionMixer-main/MotionMixer-main/checkpoints/amass', help='directory with the models checkpoints ')
    ...
    parser_mpjpe.add_argument('--tokens_mlp_dim', default=25, type=int, required=False)
