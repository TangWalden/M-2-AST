import os 
from utils_ import h36motion3d as datasets
...
import torch
import numpy as np
from utils_.loss_funcs import *
from utils_.data_utils import define_actions
from utils_.h36_3d_viz import visualize
from utils_.parser import args
  ...
  for action in actions:
    ...
    for cnt,batch in enumerate(test_loader):
      with torch.no_grad():
        ...
        i = i + 1
        print(i)
        a = all_joints_seq.view(-1,args.output_n,32,3).detach().cpu().numpy()
        b = sequences_gt.view(-1,args.output_n,32,3).detach().cpu().numpy()
        a_ = a[200, :, :, :]
        b_ = b[200, :, :, :]
        root = r'E:\STSGCN-main\STSGCN-main\vis_skeleton_200'
        path = root + '/' + str(action) + '.npy'
        path_ = root + '/' + str(action) + '_gt.npy'
        print(path)
        np.save(path, a_)
        np.save(path_, b_)
